...
export const scheduleInterview = async (applicantId: string, time: string, date: string, type: string, room:string): Promise<ApplicantsEntity | null> => {
    try {
        const status = await applicantModel.findById(applicantId)
        if(status?.hiring_status !== 'interview') {
            //!   USED TO MAKE SURE RECRUTERS CANT SCHEDULE INTERVIEW IF THE HIRING
            //! STATUS IS NOT IN INTERVIEW
